...
begin
     ...
     repeat
           ...
           begin
                RaiseError('!!!ТАЙМАУТ ПРИ ПЕРЕДАЧЕ ПАКЕТА!');
     ...
     if Assigned(CallBackDone) then CallBackDone;
     if c <> (stdSize + RespSize) then
          RaiseError('!!!НЕПРАВИЛЬНЫЙ РАЗМЕР ОТВЕТНОГО ПАКЕТА! Ожидалось: '+
                     IntToStr(c)+' байт, пришло: '+IntToStr(stdSize + RespSize)+
                     ' байт.');
