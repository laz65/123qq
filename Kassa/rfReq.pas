...
var
    ...
    M: Char;
begin
     if (GetArgNum(Cmd) < 3) or (GetArgNum(Cmd) > 4) then RaiseError('RSP: Неправильное количество аргументов');
     S := Cmd;
     Arg := GetNextArg(S);  // 9
...
begin
     S := Cmd;
     if GetArgNum(S) <> 3 then RaiseError('DPY: Неправильное количество аргументов');
     Arg := GetNextArg(S); // 2
